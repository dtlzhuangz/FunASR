...
class DecoderLayerSANM_split(torch.nn.Module):
    ...
    def __init__(
        ...
        super(DecoderLayerSANM_split, self).__init__()
        self.size = size
...
@tables.register("decoder_classes", "ParaformerSANMDecoder_split")
class ParaformerSANMDecoder_split(BaseTransformerDecoder):
    """
    ...
    def __init__(
        ...
        self.decoders = repeat(
            att_layer_num,
            lambda lnum: DecoderLayerSANM_split(
                attention_dim,
                MultiHeadedAttentionSANMDecoder(
...
class DecoderLayerSANM_splitExport(torch.nn.Module):
...
@tables.register("decoder_classes", "ParaformerSANMDecoder_splitExport")
class ParaformerSANMDecoder_splitExport(torch.nn.Module):
    def __init__(self, model, max_seq_len=512, model_name="decoder", onnx: bool = True, **kwargs):
        ...
        for i, d in enumerate(self.model.decoders):
            ...
            self.model.decoders[i] = DecoderLayerSANM_splitExport(d)

        if self.model.decoders2 is not None:
            for i, d in enumerate(self.model.decoders2):
                if isinstance(d.self_attn, MultiHeadedAttentionSANMDecoder):
                    d.self_attn = MultiHeadedAttentionSANMDecoderExport(d.self_attn)
                self.model.decoders2[i] = DecoderLayerSANM_splitExport(d)

        for i, d in enumerate(self.model.decoders3):
            self.model.decoders3[i] = DecoderLayerSANM_splitExport(d)
...
@tables.register("decoder_classes", "ParaformerSANMDecoderOnline_splitExport")
class ParaformerSANMDecoderOnline_splitExport(torch.nn.Module):
    def __init__(self, model, max_seq_len=512, model_name="decoder", onnx: bool = True, **kwargs):
        super().__init__()
        # self.embed = model.embed #Embedding(model.embed, max_seq_len)
        self.model = model
...
@tables.register("decoder_classes", "ParaformerSANDecoder_split")
class ParaformerSANDecoder_split(BaseTransformerDecoder):
    """
    Author: Speech Lab of DAMO Academy, Alibaba Group
    Paraformer: Fast and Accurate Parallel Transformer for Non-autoregressive End-to-End Speech Recognition
    https://arxiv.org/abs/2006.01713
