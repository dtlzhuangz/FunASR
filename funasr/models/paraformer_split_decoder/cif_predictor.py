...
@tables.register("predictor_classes", "CifPredictor_split_decoder")
class CifPredictor_split_decoder(torch.nn.Module):
    def __init__(
        self,
        idim,
        l_order,
...
@tables.register("predictor_classes", "CifPredictorV2_split_decoder")
class CifPredictorV2_split_decoder(torch.nn.Module):
    def __init__(
        self,
        idim,
        l_order,
...
@tables.register("predictor_classes", "CifPredictorV2_split_decoderExport")
class CifPredictorV2_split_decoderExport(torch.nn.Module):
...
def cif_export(hidden, alphas, threshold: float):
    ...
    frame = torch.zeros(
        [batch_size, hidden_size], dtype=hidden.dtype, device=hidden.device
    )
    ...
    for t in range(len_time):
        ...
        distribution_completion = (
            torch.ones([batch_size], dtype=alphas.dtype, device=hidden.device)
            - integrate
        )
