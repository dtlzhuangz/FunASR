...
from funasr.models.sanm.positionwise_feed_forward import (
...
class DecoderLayerSANM_split_decoder(torch.nn.Module):
    ...
    def __init__(
        ...
        super(DecoderLayerSANM_split_decoder, self).__init__()
        self.size = size
...
@tables.register("decoder_classes", "ParaformerSANMDecoder_split_decoder")
class ParaformerSANMDecoder_split_decoder(BaseTransformerDecoder):
    """
    ...
    def __init__(
        ...
        self.decoders = repeat(
            att_layer_num,
            lambda lnum: DecoderLayerSANM_split_decoder(
                attention_dim,
                MultiHeadedAttentionSANMDecoder(
                    attention_dim,
    ...
    def forward_asf2(
        ...
        tgt, tgt_mask, memory, memory_mask, _ = self.decoders[0](
            tgt, tgt_mask, memory, memory_mask
        )
        attn_mat = self.model.decoders[1].get_attn_mat(
            tgt, tgt_mask, memory, memory_mask
        )
        return attn_mat
    ...
    def forward_asf6(
        ...
        tgt, tgt_mask, memory, memory_mask, _ = self.decoders[1](
            tgt, tgt_mask, memory, memory_mask
        )
        tgt, tgt_mask, memory, memory_mask, _ = self.decoders[2](
            tgt, tgt_mask, memory, memory_mask
        )
        tgt, tgt_mask, memory, memory_mask, _ = self.decoders[3](
            tgt, tgt_mask, memory, memory_mask
        )
        tgt, tgt_mask, memory, memory_mask, _ = self.decoders[4](
            tgt, tgt_mask, memory, memory_mask
        )
...
class DecoderLayerSANM_split_decoderExport(torch.nn.Module):
...
@tables.register("decoder_classes", "ParaformerSANMDecoder_split_decoderExport")
class ParaformerSANMDecoder_split_decoderExport(torch.nn.Module):
    def __init__(
        self, model, max_seq_len=512, model_name="decoder", onnx: bool = True, **kwargs
    ):
        super().__init__()
        ...
        for i, d in enumerate(self.model.decoders):
            ...
            self.model.decoders[i] = DecoderLayerSANM_split_decoderExport(d)

        if self.model.decoders2 is not None:
            for i, d in enumerate(self.model.decoders2):
                if isinstance(d.self_attn, MultiHeadedAttentionSANMDecoder):
                    d.self_attn = MultiHeadedAttentionSANMDecoderExport(d.self_attn)
                self.model.decoders2[i] = DecoderLayerSANM_split_decoderExport(d)

        for i, d in enumerate(self.model.decoders3):
            self.model.decoders3[i] = DecoderLayerSANM_split_decoderExport(d)
    ...
    def forward_asf6(
        ...
        attn_mat = self.model.decoders[5].get_attn_mat(
            tgt, tgt_mask, memory, memory_mask
        )
        return attn_mat
...
@tables.register("decoder_classes", "ParaformerSANMDecoderOnline_split_decoderExport")
class ParaformerSANMDecoderOnline_split_decoderExport(torch.nn.Module):
    def __init__(
        self, model, max_seq_len=512, model_name="decoder", onnx: bool = True, **kwargs
    ):
        super().__init__()
    ...
    def get_dummy_inputs(self, enc_size):
        ...
        cache = [
            torch.zeros(
                (
                    2,
                    self.model.decoders[0].size,
                    self.model.decoders[0].self_attn.kernel_size - 1,
                ),
                dtype=torch.float32,
            )
            for _ in range(cache_num)
...
@tables.register("decoder_classes", "ParaformerSANDecoder_split_decoder")
class ParaformerSANDecoder_split_decoder(BaseTransformerDecoder):
    """
    Author: Speech Lab of DAMO Academy, Alibaba Group
    Paraformer: Fast and Accurate Parallel Transformer for Non-autoregressive End-to-End Speech Recognition
    https://arxiv.org/abs/2006.01713
        ...
        cache = [
            torch.zeros(
                (
                    1,
                    self.model.decoders[0].size,
                    self.model.decoders[0].self_attn.kernel_size,
                )
            )
            for _ in range(cache_num)
        ]
